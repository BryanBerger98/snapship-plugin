---
name: init
description: Bootstrap an snap workspace in the current project — detect platforms, ask the user to confirm, write `snapship.config.json`, scaffold `.snap/`. Run once before `/snap:define`.
...
project-level config (`snapship.config.json`) and scaffolds the local workspace
...
| 01 | `step-01-write.md`  | Write `snapship.config.json`, scaffold `.snap/`, validate |
