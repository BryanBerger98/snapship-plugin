...
echo "[1] start"
DIR=$(setup_dir)
bash "$SCRIPT" start --skill=define --story-id=01-auth --project-root="$DIR"
...
[ "$(jq -r '.in_flight[0].story_id' "$F")" = "01-auth" ] && ok "1.5 story_id" || ko "1.5"
...
DIR=$(setup_dir)
bash "$SCRIPT" start --skill=define --story-id=01-auth --project-root="$DIR"
bash "$SCRIPT" start --skill=ticket --story-id=01-auth --project-root="$DIR"
F="${DIR}/.snap/progress.json"
[ "$(jq '.in_flight | length' "$F")" = "2" ] && ok "3.1 two entries" || ko "3.1"
trash "$DIR" 2>/dev/null || true

# 4. invalid story-id rejected
echo ""
echo "[4] invalid story-id"
DIR=$(setup_dir)
if bash "$SCRIPT" start --skill=define --story-id=BAD --project-root="$DIR" 2>/dev/null; then
  ko "4.1 should reject"
else
  ok "4.1 rejected invalid story-id"
fi
# _global accepted
bash "$SCRIPT" start --skill=init --story-id=_global --project-root="$DIR" && ok "4.2 _global accepted" || ko "4.2"
trash "$DIR" 2>/dev/null || true
...
DIR=$(setup_dir)
bash "$SCRIPT" start --skill=define --story-id=01-auth --project-root="$DIR"
bash "$SCRIPT" step --skill=define --story-id=01-auth --step-num=01 --step-name=init --status=started --project-root="$DIR"
F="${DIR}/.snap/progress.json"
...
bash "$SCRIPT" step --skill=define --story-id=01-auth --step-num=01 --step-name=init --status=ok --project-root="$DIR"
...
# new step appended
bash "$SCRIPT" step --skill=define --story-id=01-auth --step-num=02 --step-name=brainstorm --status=ok --project-root="$DIR"
...
DIR=$(setup_dir)
bash "$SCRIPT" start --skill=define --story-id=01-auth --project-root="$DIR"
if bash "$SCRIPT" step --skill=define --story-id=01-auth --step-num=01 --step-name=init --status=bogus --project-root="$DIR" 2>/dev/null; then
...
DIR=$(setup_dir)
bash "$SCRIPT" step --skill=define --story-id=01-auth --step-num=01 --step-name=init --status=ok --note="ok done" --extra='{"a":1}' --project-root="$DIR"
F="${DIR}/.snap/progress.json"
...
DIR=$(setup_dir)
bash "$SCRIPT" start --skill=define --story-id=01-auth --project-root="$DIR"
bash "$SCRIPT" finish --skill=define --story-id=01-auth --status=ok --project-root="$DIR"
F="${DIR}/.snap/progress.json"
...
DIR=$(setup_dir)
bash "$SCRIPT" start --skill=define --story-id=01-auth --project-root="$DIR"
bash "$SCRIPT" finish --skill=define --story-id=01-auth --status=fail --project-root="$DIR"
F="${DIR}/.snap/progress.json"
...
DIR=$(setup_dir)
out=$(bash "$SCRIPT" resume --skill=define --story-id=01-auth --project-root="$DIR")
...
DIR=$(setup_dir)
bash "$SCRIPT" step --skill=define --story-id=01-auth --step-num=01 --step-name=init --status=ok --project-root="$DIR"
bash "$SCRIPT" step --skill=define --story-id=01-auth --step-num=02 --step-name=brainstorm --status=fail --project-root="$DIR"
out=$(bash "$SCRIPT" resume --skill=define --story-id=01-auth --project-root="$DIR")
