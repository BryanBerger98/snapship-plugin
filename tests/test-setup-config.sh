...
[ -f "$TMP/artysan.config.json" ] && ok "5.1 file exists" || ko "5.1"
[ "$out" = "$TMP/artysan.config.json" ] && ok "5.2 stdout = path" || ko "5.2"
v=$(jq -r '.version' "$TMP/artysan.config.json")
...
[ "$(cat "$TMP/artysan.config.json")" = "{}" ] && ok "6.2 file untouched" || ko "6.2"
trash "$TMP" 2>/dev/null || rm -rf "$TMP"
...
[ "$(jq -r '.tickets.platform' "$TMP/artysan.config.json")" = "jira" ] && ok "8.1 tickets jira" || ko "8.1"
[ "$(jq -r '.defaults.lang' "$TMP/artysan.config.json")" = "en" ] && ok "8.2 lang en" || ko "8.2"
[ "$(jq -r '.repository.platform' "$TMP/artysan.config.json")" = "github" ] && ok "8.3 repo unchanged" || ko "8.3"
trash "$TMP" 2>/dev/null || rm -rf "$TMP"
...
[ "$(jq -r '.ai.max_parallel_agents' "$TMP/artysan.config.json")" = "3" ] && ok "9.1 nested merged" || ko "9.1"
[ "$(jq -r '.repository.platform' "$TMP/artysan.config.json")" = "github" ] && ok "9.2 base preserved" || ko "9.2"
trash "$TMP" 2>/dev/null || rm -rf "$TMP"
...
[ -f "$TMP/artysan.config.json" ] && ok "11.1 written" || ko "11.1"
trash "$TMP" 2>/dev/null || rm -rf "$TMP"
...
if command -v ajv >/dev/null 2>&1; then
  ...
  if ajv validate --spec=draft2020 --strict=false -s "$SCHEMA" -d "$TMP/artysan.config.json" >/dev/null 2>&1; then
