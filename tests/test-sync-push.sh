...
DIR=$(setup_dir)
init_repo "$DIR"
p=$(bash "$SCRIPT" staging-path --feature-id=01-auth --kind=prd --project-root="$DIR")
[ "$p" = "${DIR}/.snap/PRDs/01-auth.md" ] && ok "1.1 prd path" || ko "1.1 got: $p"
p=$(bash "$SCRIPT" staging-path --feature-id=01-auth --kind=tickets --project-root="$DIR")
[ "$p" = "${DIR}/.snap/tickets/01-auth.json" ] && ok "1.2 tickets path" || ko "1.2 got: $p"
p=$(bash "$SCRIPT" staging-path --feature-id=01-auth --kind=design-gallery --project-root="$DIR")
[ "$p" = "${DIR}/.snap/designs/01-auth" ] && ok "1.3 design-gallery path" || ko "1.3 got: $p"
p=$(bash "$SCRIPT" staging-path --feature-id=01-auth --kind=design-gallery --screen=login.png --project-root="$DIR")
...
out=$(bash "$SCRIPT" ack --feature-id=01-auth --kind=prd --platform=affine --url=https://example.com/p --page-id=page-1 --project-root="$DIR")
...
init_repo "$DIR"
mkdir -p "${DIR}/.snap/PRDs" && echo "PRD" > "${DIR}/.snap/PRDs/01-auth.md"
bash "$SCRIPT" ack --feature-id=01-auth --kind=prd --platform=affine --url=u --no-trash --project-root="$DIR" >/dev/null
...
DIR=$(setup_dir)
init_repo "$DIR"
if bash "$SCRIPT" ack --feature-id=01-auth --kind=prd --url=u --project-root="$DIR" 2>/dev/null; then
  ko "6.1 should reject missing platform"
else
  ok "6.1 missing platform rejected"
fi
if bash "$SCRIPT" ack --feature-id=01-auth --kind=prd --platform=affine --project-root="$DIR" 2>/dev/null; then
...
init_repo "$DIR"
mkdir -p "${DIR}/.snap/PRDs" && echo "PRD" > "${DIR}/.snap/PRDs/01-auth.md"
out=$(bash "$SCRIPT" fail --feature-id=01-auth --kind=prd --note="MCP timeout" --project-root="$DIR")
...
DIR=$(setup_dir)
init_repo "$DIR"
bash "$SCRIPT" mark --feature-id=01-auth --kind=prd --status=dirty --project-root="$DIR" >/dev/null
M="${DIR}/.snap/manifests/01-auth.manifest.json"
[ "$(jq -r '.refs.prd.sync_status' "$M")" = "dirty" ] && ok "8.1 dirty" || ko "8.1"
if bash "$SCRIPT" mark --feature-id=01-auth --kind=prd --status=bogus --project-root="$DIR" 2>/dev/null; then
...
bash "$SCRIPT" bogus --feature-id=01-auth --kind=prd 2>/dev/null; [ $? -eq 1 ] && ok "10.3 unknown subcmd = 1" || ko "10.3"
