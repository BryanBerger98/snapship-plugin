...
# deterministic shell-level contracts.
...
in_review=$(jq -r '[.tickets[] | select(.status=="in_review") | .local_id] | join(",")' "$TICKETS")
...
sha=$(jq -r '.tickets[] | select(.local_id=="t-001").commit_sha' "$TICKETS")
[ -n "$sha" ] && [ "$sha" != "null" ] \
...
ac1=$(jq -r '.tickets[] | select(.local_id=="t-001").acceptance_criteria[0].checked' "$TICKETS")
ac2=$(jq -r '.tickets[] | select(.local_id=="t-001").acceptance_criteria[1].checked' "$TICKETS")
assert_eq "02.7 AC#1 pass → checked=true"  "true"  "$ac1"
assert_eq "02.8 AC#2 fail → checked=false" "false" "$ac2"

ac1_text=$(jq -r '.tickets[] | select(.local_id=="t-001").acceptance_criteria[0].text' "$TICKETS")
assert_eq "02.9 AC text preserved" "Form renders" "$ac1_text"

# --- step-03 cycle counter ------------------------------------------------
echo ""
...
cyc=$(jq -r '.tickets[] | select(.local_id=="t-001").qa_cycles_used' "$TICKETS")
...
new_status=$(jq -r '.tickets[] | select(.local_id=="t-001").status' "$TICKETS")
assert_eq "05.6 status=qa-validated"      "qa-validated"          "$new_status"
new_ts=$(jq -r '.tickets[] | select(.local_id=="t-001").qa_validated_at' "$TICKETS")
