...
- **Figma** : token chargé depuis `.env.snapship` racine projet (clé par défaut `FIGMA_ACCESS_TOKEN`, override `design.figma.token_env`) via `skills/_shared/load-env.sh`. Exporté dans l'env pour `figma-console-mcp` + `bridge-ds`. Fichier absent ou clé absente → halt clair. `figma-helper.sh get-current-file` compare `figma.fileKey` à `design.figma.file_key`. Mismatch → halt. CLI `bridge-ds` joignable (sinon `npm i -g @noemuch/bridge-ds`).
...
3. **Token Figma** dans `.env.snapship` racine projet (clé `FIGMA_ACCESS_TOKEN` par défaut, override `design.figma.token_env`). Fichier gitignored. Voir [docs/config.md → Secrets](../config.md#secrets--envsnapship).
