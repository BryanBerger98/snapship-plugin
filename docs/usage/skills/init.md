...
confirmation, writes `snap.config.json`, and creates the `.snap/`
tree.
...
`snap.config.json` is missing and points back here.
...
- **Re-init**: `--force` rewrites an existing `snap.config.json`
...
| `--force`        | Rewrites an existing `snap.config.json`. Safe: does not touch `.snap/`.                             |
...
| 01 | `step-01-write.md`  | Writes `snap.config.json`, creates `.snap/`, validates against the schema. |
